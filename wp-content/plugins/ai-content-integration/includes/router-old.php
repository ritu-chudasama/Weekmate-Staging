...
function wmcp_generate_city_page($city, $state, $page_type = 'payroll')
{
	...
	if (is_wp_error($data)) {
		wmcp_log('Groq API failed — using static fallback', array('error' => $data->get_error_message(), 'city' => $city, 'page_type' => $page_type));
		delete_transient($lock_key);
		$data = wmcp_get_fallback_page_data($page_type);
	}
...
function wmcp_template_redirect()
{
	...
	// Geolocation failed — redirect to static fallback page.
	if ( empty( $city ) ) {
		wmcp_log( 'geolocation failed — redirecting to fallback', array( 'page_type' => $page_type ) );
		$config = wmcp_get_page_type_definition( $page_type );
		$slug   = ( $config['slug_prefix'] ?? 'payroll-software-in-' ) . 'indian-businesses';
