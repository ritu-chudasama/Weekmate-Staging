...
function wmcp_apply_fixed_headings($data, $city, $state, $page_type = 'payroll')
{
	...
	if ('hr' === $page_type) {
		...
		$data['meta']['h1'] = 'HR Software in ' . $city .  '— Built for India\'s Fastest-Moving Workforce';
		...
		// --- places ---
		if (empty($data['sections']['places']['h2'])) {
			$data['sections']['places']['h2'] = 'Top 5 Places to Visit in ' . $city;
		}
		if (empty($data['sections']['places']['h3'])) {
			$data['sections']['places']['h3'] = 'A quick snapshot of the city your team calls home' . $city;
		}

		// --- pain points ---
		$data['sections']['pain_points']['h2'] = 'HR Challenges ' . $city . 'Businesses Face And How WeekMate Solves!';
		...
		$data['meta']['h1'] = 'Payroll Software in ' . $city . '— Built for India\'s Fastest-Moving Workforce';
		...
		if (empty($data['sections']['places']['h3'])) {
			$data['sections']['places']['h3'] = 'Key commercial hubs and business districts in ' . $city;
		}
