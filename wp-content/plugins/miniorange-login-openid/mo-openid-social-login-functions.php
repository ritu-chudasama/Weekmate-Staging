...
function create_customer($password) {
	if ( ! wp_verify_nonce( $nonce, 'mo-openid-connect-register-nonce' ) ) {
		wp_die( '<strong>ERROR WPSL28</strong>: Please Go back and Refresh the page and try again!<br/>If you still face the same issue please contact your Administrator.' );
	} else {
...
function mo_openid_activation_message() {
	...
	$class   = 'updated';
	$message = get_option( 'mo_openid_message' );
	echo "<div class='" . esc_attr( $class ) . "'> <p>" . wp_kses( $message,$allowed_html ) . '</p></div>';
}
...
function mo_openid_check_allowed_role_map(){
	$custom_attr_list = get_option('mo_openid_custom_field_mapping');
	$allowed_arr = array(); // Initialize the array
	
	foreach ($custom_attr_list as $key => $value) {
		if ($value[1] == "user_role") {
			$role_list = explode(';', $value[4]);
			foreach ($role_list as $role) {
				array_push($allowed_arr, $role);
			}
		}
	}
	...
	$filteredArray = array_values(array_filter(array_unique($allowed_arr), function($value) use ($searchTerm) {
