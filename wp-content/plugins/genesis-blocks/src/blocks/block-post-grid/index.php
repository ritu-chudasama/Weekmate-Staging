 ...
 */
function genesis_blocks_render_block_core_latest_posts( $attributes ) {
	...
	if ( isset( $attributes['postType'] ) && 'page' === $attributes['postType'] ) {
		...
		if ( ! empty( $page_selection ) ) {
			$args['posts_per_page'] = count( $page_selection ); // phpcs:ignore WordPress.WP.PostsPerPage.posts_per_page_posts_per_page
		}
	} else {
		/* Post query args */
		$args = array(
	...
	if ( $grid_query->have_posts() ) {
		...
		while ( $grid_query->have_posts() ) {
			...
			$post_classes = 'gb-post-grid-item';
			...
			/* Adjust columns without featured image */
			if ( isset( $attributes['displayPostImage'] ) && $attributes['displayPostImage'] === false ) {
				$post_classes .= ' hide-featured-image';
			} else {
				$post_classes .= null;
			}
			...
			if ( isset( $attributes['displayPostImage'] ) && $attributes['displayPostImage'] && $post_thumb_id ) {
				...
				$post_grid_markup .= sprintf(
					'<div class="gb-block-post-grid-image"><a href="%1$s" rel="bookmark" aria-hidden="true" tabindex="-1">%2$s</a></div>',
			...
			if ( ! $title ) {
				$title = __( 'Untitled', 'genesis-blocks' );
			}
			...
			if ( isset( $attributes['postType'] ) && $attributes['postType'] === 'post' ) {
				...
				if ( isset( $attributes['displayPostAuthor'] ) && $attributes['displayPostAuthor'] ) {
					$post_grid_markup .= sprintf(
						'<div class="gb-block-post-grid-author" itemprop="author" itemtype="https://schema.org/Person"><a class="gb-text-link" href="%2$s" itemprop="url" rel="author"><span itemprop="name">%1$s</span></a></div>',
				...
				if ( isset( $attributes['displayPostDate'] ) && $attributes['displayPostDate'] ) {
						$post_grid_markup .= sprintf(
							'<time datetime="%1$s" class="gb-block-post-grid-date" itemprop="datePublished">%2$s</time>',
			...
			if ( isset( $attributes['displayPostLink'] ) && $attributes['displayPostLink'] ) {
				$post_grid_markup .= sprintf(
					'<p><a class="gb-block-post-grid-more-link gb-text-link" href="%1$s" rel="bookmark">%2$s <span class="screen-reader-text">%3$s</span></a></p>',
					esc_url( get_permalink( $post_id ) ),
		...
		$class = "gb-block-post-grid featured{$attributes['postType']} align{$attributes['align']}";
 ...
 */
function genesis_blocks_register_block_core_latest_posts() {
	...
	register_block_type(
		...
		array(
			...
			'render_callback' => 'genesis_blocks_render_block_core_latest_posts',
		)
	);
}
add_action( 'init', 'genesis_blocks_register_block_core_latest_posts' );
