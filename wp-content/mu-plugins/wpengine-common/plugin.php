...
class WpeCommon extends WpePlugin_common {
	...
	public function get_error_log_url( $production = true ) {
        	...
        	if ( is_null( $wpengine_platform_config ) || ! isset( $wpengine_platform_config['locations']['api1'] ) ) {
        		return '';
        	}
    ...
    public function php_version_cookie( $version_string = '7.2' ) {
        $cookie_expire    = '0';
        $cookie_path      = '/';
        if ( $version_string && preg_match( '#^[0-9]\.[0-9]$#', $version_string ) ) {
          $_COOKIE[ $this::PHP_VERSION_COOKIE_NAME ] = $version_string;
          # CA-5006: Don't set httponly attribute on cookie as it prevents the cookie being unset by javascript
          setcookie( $this::PHP_VERSION_COOKIE_NAME, $version_string, $cookie_expire, $cookie_path, '', true, false );
        }
    }
	...
	public function display_php_version_admin_notice() {
		$php_version = $_COOKIE[ $this::PHP_VERSION_COOKIE_NAME ] ?? '';
		if ( $php_version && preg_match( '#^[0-9]\.[0-9]$#', $php_version ) && strpos( phpversion(), $php_version ) === 0 ) {
			global $wpengine_platform_config;
	...
	if ( $cdn_domain && $cdn_enabled && ! $is_admin ) {  // XXX: DISABLED FOR ADMINS BECAUSE OF THESITEWHICHWILLNOTBENAMED.COM -- USE OUR OWN CDN TO FIX!
        ...
        if ( defined( 'WPE_EXTERNAL_URL' ) && WPE_EXTERNAL_URL ) {
    ...
    public function get_cdn_domain( $netdna_config, $blog_url, $is_ssl ) {
        ...
        // netdna-ssl.com is now owned by an unrelated third party (CSI-7875), so SSL zone-only
        // configs fall back to origin rather than build a URL against it. Only explicit custom
        // domains (or an FQDN zone key) are safe for SSL. Non-SSL below still uses netdna-cdn.com,
        // which is unaffected.
        if ( $is_ssl ) {
            foreach ( $netdna_config as $zone => $domain ) {
                if ( is_array( $domain ) ) {
                    if ( isset( $domain['match'] ) && isset( $domain['custom'] ) && 0 == strcasecmp( $blog_domain, $domain['match'] ) )
                        return $domain['custom'];
                } else if ( 0 == strcasecmp( $blog_domain, $domain ) && strpos( $zone, '.' ) !== false ) {
                    return $zone;
                }
            }
            return false;
     ...
     * @param uri e.g. "/v2/do_thing?apikey=1234"
     * @param extra_headers associative array of headers to add to the request
     * @param payload data to send with the POST request
     ...
     */
    public static function http_request_async( $method, $domain, $port, $hostname, $uri, $extra_headers = array( ), $wait_ms = 100, $payload = null ) {
